...
awk '
function val(key, p,s) { p=key "=[^ ]+"; if(match($0,p)){s=RSTART+length(key)+1; return substr($0,s,RLENGTH-length(key)-1)} return "-" }
function qval(key, p,s) { p=key "=\"[^\"]*\""; if(match($0,p)){s=RSTART+length(key)+2; return substr($0,s,RLENGTH-length(key)-3)} return "" }
function add(g,n,v) { total[g SUBSEP n]+=v }
function grp(a,s,slot,cfg,sym) { return a "|" s "|" slot "|" cfg "|" sym }
...
/\[GAP_STABILITY\]\[(CYCLE_STABLE|CYCLE_COMPLETED|TRIGGER_EMITTED)\]/ {
 ...
 policy[g,"absolute_floor"]=val("absolute_floor");policy[g,"relative_tolerance"]=val("relative_tolerance");policy[g,"mad_multiplier"]=val("mad_multiplier");policy[g,"min_stable_samples"]=val("min_stable_samples");policy[g,"max_dispersion"]=val("max_dispersion");policy[g,"max_drift"]=val("max_drift");policy[g,"hold_confirm_ms"]=val("hold_confirm_ms");policy[g,"limit_max_gap"]=val("limit_max_gap");policy[g,"max_gap"]=val("max_gap")
...
END {
 ...
 for(g in groups){split(g,part,/[|]/);print "";print "group";print "action," part[1];print "side," part[2];print "slot_id," part[3];print "config_id," part[4];print "symbol," part[5]
