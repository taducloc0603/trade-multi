...
public sealed class DashboardMetricsMapperTests
{
    ...
    // Gap A-C: Buy = C.Bid - A.Ask = 3, Sell = A.Bid - C.Ask = -7.
    // Gap B-C: Buy = C.Bid - B.Ask = -7, Sell = B.Bid - C.Ask = 3.
    [Fact]
    public void Map_WithSanC_ComputesAcAndBcGaps_CBidMinusXAsk_AndXBidMinusCAsk()
    {
        ...
        // A-C: Buy = C.Bid - A.Ask / Sell = A.Bid - C.Ask.
        Assert.Equal(3, result.GapBuyAC);
        Assert.Equal(-7, result.GapSellAC);
        // B-C: Buy = C.Bid - B.Ask / Sell = B.Bid - C.Ask.
        Assert.Equal(-7, result.GapBuyBC);
        Assert.Equal(3, result.GapSellBC);
