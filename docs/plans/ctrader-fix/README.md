...
| **7** | [phase-7-execution.md](phase-7-execution.md) | **PHIÊN TIỀN THẬT duy nhất**: A) spike 721 (câu 4/1/5/6) · B) nghiệm thu 5/6 **có position** · C) executor thật | đầy đủ | **có** | 🔶 **Bước A GO trên Deriv 1551176 (2026-09-23)** — FxPro 8220816 CHẶN đặt lệnh qua FIX (`CHANNEL_IS_BLOCKED`, P7-A1); Deriv khớp lệnh 284 ms và đóng bằng `721` 101 ms, `728=2` (P7-A3). Chi phí thật −$3,55. Bước B (phần đọc) ĐẠT trên Deriv; Bước C code xong + đã bật DI nhưng **nghiệm thu live CHƯA chạy** (31 checkbox) — sẽ chạy trên VPS, xem phase-7-vps-run.md. |
| 8 | [phase-8-hardening.md](phase-8-hardening.md) | Cảnh báo, Telegram, tài liệu | đầy đủ | có | ☐ **Chưa bắt đầu** — phụ thuộc Phase 7 xong (cảnh báo phải phản ánh hành vi đã ship). |
