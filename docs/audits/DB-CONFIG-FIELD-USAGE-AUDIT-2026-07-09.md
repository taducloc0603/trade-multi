...
| `min_profit_to_close` | Trị tuyệt đối dịch chuyển chân A từ Open Price tối thiểu (point) để Auto Close khi tuổi lệnh còn dưới `max_life_time_by_second`. `0` là tắt; khi đạt max lifetime thì bỏ qua ngưỡng nhưng vẫn cần close signal. |
...
> UI Trading Signal hiển thị quota Old gần nhất, Current, Cycle, Progress và trạng thái Active/OVER TARGET.
